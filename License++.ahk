...
if (!CheckHwid()) { ; Здесь происходит проверка на hwid, если функция вернула false, то он выводит сообщение что нет hwid на хостинге, если true пропускается
    MsgBox, Добро пожаловать в поле регистрации!`nСкопированные данные отправьте @GraffFort (TG).
...
msgbox, 0x40, Информация, % "Добро Пожаловать, " name "!`nСофт Активирован!`nАвтор: @GraffFort (TG)`nВы Присоединились к Нам " regdate "!`nМы Всегда Свяжимся с Вами В Случае Проблем По Этим Данным: " forum "!"